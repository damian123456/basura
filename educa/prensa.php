<?php
    ...
    $tmp = $db->fetch_all("SELECT * FROM galerias WHERE categoria=1 ORDER BY orden, id desc");
...
$tmp = $db->fetch_all("SELECT * FROM galerias WHERE categoria=1 ORDER BY orden, id desc LIMIT ".$inicio."," . $TAMANO_PAGINA);
    if(is_array($tmp)) {
        $galerias = $tmp;
        foreach($galerias as $key => $g){
            $tmp = $db->fetch_item("
...
<!--[if gt IE 8]><!--> <html class="no-js"> <!--<![endif]-->
    <head>
        <meta charset="utf-8">
        <title>Prensa Educa Idiomas</title>
...
.container-galeria div {
 ...
 .container-galeria img {
        ...
        <div class="container">
            ...
            <div class="row">
                ...
                <?php             
						...
						echo "<a href='galeria_detalle_prensa.php?id=". $g['id']."'>";
   ...
   if ($pagina != 1) {
       echo '<li><a href="prensa.php?pagina='.($pagina-1).'"><span aria-hidden="true">Anterior</span></a></li>';
   }
   for ($i=1;$i<=$total_paginas;$i++) {
         if ($pagina == $i) {
            ...
            echo '<li>  <a href="prensa.php?pagina='.$i.'">'.$i.'</a> </li> ';
            }
   }
      if ($pagina != $total_paginas)
         echo '<li><a href="prensa.php?pagina='.($pagina+1).'"><span aria-hidden="true">Siguiente</span></a></li>';
}?>
