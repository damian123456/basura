<?php
    ...
    $tmp = $db->fetch_all("SELECT * FROM galerias WHERE categoria=0 ORDER BY orden, id desc");
...
$tmp = $db->fetch_all("SELECT * FROM galerias WHERE categoria=0 ORDER BY orden, id desc LIMIT ".$inicio."," . $TAMANO_PAGINA);
    if(is_array($tmp)) {
        $galerias = $tmp;
        foreach($galerias as $key => $g){
            $tmp = $db->fetch_item("
...
<!--[if gt IE 8]><!--> <html class="no-js"> <!--<![endif]-->
    <head>
        <meta charset="utf-8">
        <title>Galeria de Fotos Instituto Educa Idiomas Mar del Plata </title>
...
.container-galeria div {
 ...
 .container-galeria img {
   ...
   if ($pagina != 1) {
       echo '<li><a href="galeria.php?pagina='.($pagina-1).'"><span aria-hidden="true">Anterior</span></a></li>';
   }
   for ($i=1;$i<=$total_paginas;$i++) {
         if ($pagina == $i) {
            ...
            echo '<li>  <a href="galeria.php?pagina='.$i.'">'.$i.'</a> </li> ';
            }
   }
      if ($pagina != $total_paginas)
         echo '<li><a href="galeria.php?pagina='.($pagina+1).'"><span aria-hidden="true">Siguiente</span></a></li>';
}?>
