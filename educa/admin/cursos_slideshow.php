<?
	...
	if($id){
		$item = $db->fetch_item("SELECT * FROM cursos_slideshow WHERE id=$id");
	}
	...
	if($_POST['save']){
		...
		if(!$title || !$subtitle || !$details || !$green_btn_text || !$green_btn_url)
		...
		if(empty($errores)){
			...
			if($item){
				...
				$db->update("UPDATE cursos_slideshow SET {$fields},added=NOW() WHERE id = {$id}");
			} else {
				$insert = $db->insert("INSERT INTO cursos_slideshow SET {$fields},added=NOW()");
			}
			if($_POST['apply']){
				redirect("cursos_slideshow.php?id={$id}");
			} else {
				redirect("cursos_slideshow.php");
			}
	...
	if($_GET['action'] == 'delete'){
		...
		if($item){
			@unlink($upload_dir.$item['image']);
			$db->delete("DELETE FROM cursos_slideshow WHERE id={$item['id']}");
...
<?
				if($item || $action=='new'){
					include 'templates/cursos_slideshow_add.php';
				} else {
					// Traemos los banners
					$items = $db->fetch_all("
						SELECT *
						FROM cursos_slideshow
						ORDER BY position, id
					");
					include 'templates/cursos_slideshow_list.php';
				}
?>
