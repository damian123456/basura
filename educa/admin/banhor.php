...
<?
    ...
    if($id){
        $item = $db->fetch_item("SELECT * FROM banhor WHERE id=$id");
    }
    ...
    if($_POST['save']){
        ...
        if(empty($errores)){
            ...
            if($item){
                ...
                $db->update("UPDATE banhor SET {$fields}, modified=NOW() WHERE id = {$id}");
            } else {
                $insert = $db->insert("INSERT INTO banhor SET {$fields}, added=NOW()");
            }
            if($_POST['apply']){
                redirect("banhor.php?id={$id}");
            } else {
                redirect("banhor.php");
            }
...
<?
            switch($action){
                case 'new': // Si estamos agregando un nuevo banner mostramos el formulario
                    include 'templates/banhor_add.php'; 
                    break;
                
                case 'edit': // Si estamos agregando un nuevo banner mostramos el formulario
                    $id_banhorr = (int) $_GET['id'];
                    $banhorr = $db->fetch_item("SELECT * FROM banhor WHERE id = {$id}");
                    include 'templates/banhor_add.php';
                    break; 
                ...
                case 'delete':
                    ...
                    if($id_banhorr>0){
                        $db->delete("DELETE FROM banhor WHERE id = {$id_banhorr}");

                        if(!$db->error())
                            redirect("banhor.php?msg_code=3");
                        else
                            $errores[] = $db->error();
                    }
                    break;
...
?>
                <div class="well row" style="margin-left:0;margin-right:0;background:#f0f0f0">
                    ...
                    <div class="col-sm-6" style="text-align:right">
                        <a href="banhor.php?action=new" class="btn btn-primary"><i class="fa fa-plus"></i>  Añadir banner</a>
                    </div>
                </div>
...
<?              if($banhor){   ?>
                    <table id="sort" class="table table-striped">
                        <!-- On rows -->
                        <tbody>
<?                          foreach($banhor as $k => $banhorr){   ?>
                            <tr id="noticia_<?=$banhorr['id']?>">
                                <td class="hidden-xs"><img src="<?=$upload_dir.$banhorr['imagen']?>" class="img-responsive" /></td>
                                <td width="80%"><?=$banhorr['titulo']?><br/><small><?=$banhorr['contenido']?></small></td>
                                <td class="center"><a href="http://educaidiomas.com.ar" target="_blank" class="btn btn-primary"><span class="fa fa-globe"></span></a></td>
                                <td class="center"><a href="banhor.php?action=edit&id=<?=$banhorr['id']?>" class="btn btn-primary"><span class="glyphicon glyphicon-wrench"></span></a></td>
                                <td class="center"><a href="banhor.php?action=delete&id=<?=$banhorr['id']?>" class="btn btn-danger" onClick="return confirm('¿Seguro que desea eliminar este banner?');"><span class="glyphicon glyphicon-trash"></span></a></td>
                            </tr>
<?                          }
                 ?>
                        </tbody>
...
<?              }else{   ?>
                <p class="text-center">No hay banners horizontales.</p>
<?              }   ?>
<?          }   ?>
                </div>
            </div>
