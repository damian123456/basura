...

{
	...
	function set_config(){
		...
		$this->mega_debrid_eu = $this->config['mega_debrid_eu'];
	}
	function isadmin(){
...
{ 
	...
	function get($url)
	{
		...
		if(!$link){
			$user = ''; $pass = ''; $cookie = '';
			if($this->mega_debrid_eu)  include ('./hosts/mega-debrid_eu.php'); 
		}
		...
		elseif($this->longurl){
			if(function_exists("apache_get_modules") && in_array('mod_rewrite',@apache_get_modules())) $linkdown = 'http://'.$sv_name.'?file='.$job['hash'];
...

{
	...
	{
		if (strpos($url, "4shared.com")) $site = "4shared";
		elseif (strpos($url, "asfile.com")) $site = "asfile";
		elseif (strpos($url, "bitshare.com")) $site = "bitshare";
		elseif (strpos($url, "depositfiles.com") || strpos($url, "dfiles.eu")) $site = "depositfiles";
		elseif (strpos($url, "extabit.com")) $site = "extabit";
		elseif (strpos($url, "filefactory.com")) $site = "filefactory";
		elseif (strpos($url, "filepost.com")) $site = "filepost";
		elseif (strpos($url, "hotfile.com")) $site = "hotfile.com";
		elseif (strpos($url, "lumfile.com")) $site = "lumfile";
		elseif (strpos($url, "mediafire.com")) $site = "mediafire";
		elseif (strpos($url, "megashares.com")) $site = "megashares";
		elseif (strpos($url, "netload.in")) $site = "netload";
		elseif (strpos($url, "rapidgator.net")) $site = "rapidgator";
		elseif (strpos($url, "ryushare.com")) $site = "ryushare";
		elseif (strpos($url, "turbobit.net")) $site = "turbobit";
		elseif (strpos($url, "uploaded.to") || strpos($url, "ul.to") || strpos($url, "uploaded.net")) $site = "uploaded";
		elseif (strpos($url, "uploading.com")) $site = "uploading";
		elseif (strpos($url, "1fichier.com")) $site = "1fichier";
		elseif (strpos($url, "rapidshare.com")) 	$site = "rapidshare";
		elseif (strpos($url, "fshare.vn"))	   $site = "fshare";
		elseif (strpos($url, "up.4share.vn")  || strpos($url, "4share.vn"))  $site = "4share";
		elseif (strpos($url, "share.vnn.vn"))   $site = "share";
		elseif (strpos($url, "upfile.vn"))   $site = "upfile";
		else {
	...
	function convert_name($filename)
	{
		...
		if (empty($filename) == true) $filename = substr(md5(time() . $url) , 0, 15);
		return $filename;
	}
