...
var auto_refresh = setInterval(function(){$("#kwshadow2").load('index.php?infosv='+ Math.random());}, 15000);
function showOrHide(){
...
function get(obj) {
	...
	for (i=0;i< links.length;i++){
		if(links[i].length > 20 && (/http/.test(links[i]) ===true || /www/.test(links[i]) ===true)) {
			links[i] = links[i]+" ";
	...
	for (i=0;i< linkdown.length;i++){
		if (i === 100) break;
		showlink = showlink+"<div id='link"+i+"'><font color=#DEB887><b>"+linkdown[i]+"</b></font> <img src='images/loading2.gif' /></div>";
...
function ajaxget(id,url,type){
	var param = $("form[name=passlink]").serialize();
	if (type =='reget'){
	...
	setTimeout(function(){
		$.ajax({
			...
			data: "urllist="+encodeURIComponent(url)+"&"+param,
...
function donate(obj){
	if($("#accounts").val().length > 10) {
		$("#wait").html('<img src="images/loading.gif" /><BR>checking...');
		$.ajax({
			type: "POST",
...
function bbcode(type) {
	...
	if(type === "list") $("#report").text("make BB Code").show().fadeOut(3000); 
	var showlinkgen = "";
	if(navigator.appName === "Microsoft Internet Explorer"){
		...
		$.each(linkgens,function(i) {
			if(/File too big/.test(linkgens[i]) === true) {
				...
				var text = /href=\"http\:\/\/www.google\.com\.vn\/search\?q\=(.*?)\" target/g;
				var linkgen=text.exec(linkgens[i]);
				showlinkgen = showlinkgen+"[B][url=http://www.google.com.vn/search?q="+linkgen[1]+"][color=red]Link sex ??? ==&#9658; [/color][color=blue]"+linkgen[1]+"[/color][/b][/url][br]";
			}
		...
		$.each(linkgens,function(i) {
			if(/File too big/.test(linkgens[i]) === true) {
				...
				var text = /href=\"http\:\/\/www.google\.com\.vn\/search\?q\=(.*?)\"><font/g;
				var linkgen=text.exec(linkgens[i]);
				showlinkgen = showlinkgen+"[B][url=http://www.google.com.vn/search?q="+linkgen[1]+"][color=red]Link sex ??? ==&#9658; [/color][color=blue]"+linkgen[1]+"[/color][/b][/url][br]";
			}
			else if(/Link Dead/.test(linkgens[i]) === true) {
